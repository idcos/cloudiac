...
func RunClearProviderCache(c *ctx.Context) {
	...
	if count == 2 {
		if ok, _ := runner.DeleteProviderCache("./var/plugin-cache", req.Source, req.Version); ok { //nolint:gosec
			return
		}
	} else if count == 1 {
		...
		if ok, _ := runner.DeleteProviderCache("./var/plugin-cache/iac-registry.idcos.com", req.Source, req.Version); ok { //nolint:gosec
			return
		}
	}
}
