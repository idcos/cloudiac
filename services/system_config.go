...
func UpdateSystemConfig(tx *db.Session, id uint, attrs models.Attrs) (cfg *models.SystemCfg, re e.Error) {
	cfg = &models.SystemCfg{}
	if _, err := models.UpdateAttr(tx.Where("id = ?", id), &models.SystemCfg{}, attrs); err != nil {
		if e.IsDuplicate(err) {
			return nil, e.New(e.OrganizationAliasDuplicate)
		}
		return nil, e.New(e.DBError, fmt.Errorf("update sys config error: %v", err))
	}
	if err := tx.Where("id = ?", id).First(cfg); err != nil {
