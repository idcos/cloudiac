...
type PolicySuppress struct {
	...
	TargetType string `json:"targetType" gorm:"not null;comment:屏蔽目标类型" enums:"env,template,policy" example:"env-c3lcrjxczjdywmk0go90"`            // 屏蔽目标类型：env环境，template云模板，policy策略 type:enum('env','template','policy')
	...
	Type       string `json:"type" gorm:"comment:屏蔽类型" enums:"policy,source" example:"source"`                                                     // 屏蔽类型：policy按策略屏蔽，source按来源屏蔽type:enum('policy','source');
}
