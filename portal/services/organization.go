...
func getProjectResStatDataByMonth(resTypeData map[string][]ProjectOrEnvStatResult, mResTypeCount map[[2]string]int, mProjectCount map[[3]string]int, month string) []resps.ResTypeDetailStatWithUpResp {
...
func splitProjectResStatDataByMonth(dbResults []ProjectOrEnvStatResult) (map[string]map[string][]ProjectOrEnvStatResult, map[[2]string]int, map[[3]string]int) {

	// date -> resType -> data
	m := make(map[string]map[string][]ProjectOrEnvStatResult)
	now := time.Now()
	curMonth := now.Format("2006-01")
	lastMonth := now.AddDate(0, -1, 0).Format("2006-01")

	m[curMonth] = make(map[string][]ProjectOrEnvStatResult)
	m[lastMonth] = make(map[string][]ProjectOrEnvStatResult)
	...
	for _, result := range dbResults {
		...
		case curMonth:
			if m[curMonth][result.ResType] == nil {
				m[curMonth][result.ResType] = make([]ProjectOrEnvStatResult, 0)
			}
			m[curMonth][result.ResType] = append(m[curMonth][result.ResType], result)
		case lastMonth:
			if m[lastMonth][result.ResType] == nil {
				m[lastMonth][result.ResType] = make([]ProjectOrEnvStatResult, 0)
			}
	...
	for resType := range m[lastMonth] {
		if _, ok := m[curMonth][resType]; !ok {
			m[curMonth][resType] = make([]ProjectOrEnvStatResult, 0)
		}
	}
	...
	for resType := range m[curMonth] {
		if _, ok := m[lastMonth][resType]; !ok {
			m[lastMonth][resType] = make([]ProjectOrEnvStatResult, 0)
		}
	}
